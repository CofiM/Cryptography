...
namespace ZI_Projekat
{
    class Knapsack 
    {
        ...
        public List<string> Encrypt(string filePath, string textForCoding)
        {
            ...
            //{
                oneLine += textForCoding + "\n";
                string plaintext = "";
                byte[] charsInBytes = Encoding.Unicode.GetBytes(textForCoding.ToCharArray());
                BitArray B;
        ...
        public void LoadKey(string fileName)
        {
            ...
            if(fileName.CompareTo("Encrypted.txt")==0)
             {
                fileName = fileName.Replace("Encrypted.txt", "testPrivateKey.txt");
            }
            else
            {
                this.GenerateAndSaveKey(fileName);
